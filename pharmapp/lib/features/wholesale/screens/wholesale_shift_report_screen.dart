...
import '../providers/wholesale_shift_api_client.dart';

class WholesaleShiftReportScreen extends ConsumerStatefulWidget {
  const WholesaleShiftReportScreen({super.key});

  @override
  ConsumerState<WholesaleShiftReportScreen> createState() =>
      _WholesaleShiftReportScreenState();
}

class _WholesaleShiftReportScreenState
    extends ConsumerState<WholesaleShiftReportScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final query = WholesaleShiftQuery(from: _from, to: _to, staffId: user?.id);
    final shiftsAsync  = ref.watch(wholesaleShiftListProvider(query));
    final currentAsync = ref.watch(wholesaleCurrentShiftProvider);
                  ...
                  final r = await showDateRangePicker(
                    ...
                    builder: (ctx, child) => Theme(
                      data: ThemeData.dark().copyWith(
                        colorScheme: const ColorScheme.dark(primary: EnhancedTheme.accentCyan),
                      ),
                      child: child!,
                    ),
                  );
          ...
          Expanded(child: shiftsAsync.when(
            loading: () => const Center(
                child: CircularProgressIndicator(
                    color: EnhancedTheme.accentCyan, strokeWidth: 2)),
            error: (e, _) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 12),
              TextButton(
                  onPressed: () => ref.invalidate(wholesaleShiftListProvider(query)),
            ...
            data: (shifts) => shifts.isEmpty
                ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    const SizedBox(height: 12),
                    Text('No shifts for this period',
                        style: TextStyle(color: context.subLabelColor, fontSize: 14)),
                    const SizedBox(height: 4),
                    Text('Open a shift from the button above',
...
class _OpenShiftButton extends ConsumerWidget {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _ShiftSheet(
        ...
        onSubmit: () async {
          ...
          Navigator.pop(ctx);
          final ok = await ref
              .read(wholesaleShiftNotifierProvider.notifier)
  ...
  void _showCloseSheet(BuildContext context, WidgetRef ref, Shift current) {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _ShiftSheet(
        ...
        cashCtrl: cashCtrl,
        extraInfo:
            'Shift sales: ${fmtN(current.totalSales)}  ·  ${current.salesCount} transactions',
        onSubmit: () async {
          final cash = double.tryParse(cashCtrl.text.trim()) ?? 0.0;
          Navigator.pop(ctx);
          final ok = await ref
              .read(wholesaleShiftNotifierProvider.notifier)
              .closeShift(shiftId: current.id, closingCash: cash);
          if (context.mounted) {
  ...
  void _showSnack(BuildContext context, String msg, bool ok) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      backgroundColor:
          (ok ? EnhancedTheme.successGreen : EnhancedTheme.errorRed).withValues(alpha: 0.92),
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Active Wholesale Shift',
                    style: TextStyle(
                        color: EnhancedTheme.accentCyan, fontSize: 11, fontWeight: FontWeight.w600)),
                Text(
                    '${shift.staffName}  ·  ${shift.salesCount} orders  ·  ${fmtN(shift.totalSales)}',
                    style: TextStyle(
...
class _ShiftCardState extends State<_ShiftCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final color  = isOpen ? EnhancedTheme.accentCyan : EnhancedTheme.primaryTeal;
                ...
                Padding(
                  ...
                  child: Row(children: [
                    Container(
                      ...
                      child: Icon(
                          isOpen ? Icons.radio_button_checked : Icons.schedule_rounded,
                          color: color, size: 18),
                    ),
                    const SizedBox(width: 12),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
