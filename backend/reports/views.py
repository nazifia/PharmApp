...
def sales_report(request):
    ...
    total_retail    = float(retail_sales.aggregate(
        t=db_models.Sum('total_amount'))['t'] or 0)
    total_wholesale = float(wholesale_sales.aggregate(
        t=db_models.Sum('total_amount'))['t'] or 0)
    total_revenue   = total_retail + total_wholesale

    # Top items by qty sold
    top_items_qs = (SaleItem.objects
...
def inventory_report(request):
    ...
    stock_value = float(items.aggregate(
        v=db_models.Sum(db_models.F('stock') * db_models.F('price'),
                        output_field=db_models.FloatField()))['v'] or 0)
...
def customer_report(request):
    ...
    total_debt = float(all_customers.aggregate(
        d=db_models.Sum('outstanding_debt'))['d'] or 0)
...
def profit_report(request):
    ...
    sales = Sale.objects.filter(created__date__gte=start, created__date__lte=end)
    revenue = float(sales.aggregate(
        t=db_models.Sum('total_amount'))['t'] or 0)

    # Simple: assume 30% margin for profit estimate
    profit = revenue * 0.30
